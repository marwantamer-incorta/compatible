...
class Form extends React.Component<FormProps, any> {
  ...
  componentDidMount() {
    try {
      warning(
        getComputedStyle(document.querySelector('.ant-col'), null).getPropertyValue('position') ===
          'relative',
        'Form',
...
const FormFC = React.forwardRef((props: FormProps, ref: any) => {
  ...
  return wrapSSR(
    <Form {...props} ref={ref} prefixCls={prefixCls} className={classNames(className, hashId)} />,
  );
}) as React.ForwardRefExoticComponent<FormProps & React.RefAttributes<any>> & {
